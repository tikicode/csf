...
# Loop through the combinations for the swim_trace
for s_size in $set_sizes; do
    for b_size in $block_sizes; do
        for byte_s in $byte_sizes; do
            for wp in "${write_policy[@]}"; do
                for wt in "${write_through[@]}"; do
                    for cache in "${cache_type[@]}"; do
                            output_file="${s_size}_${b_size}_${byte_s}_${wp}_${wt}_${cache}.txt"
                            echo "Running ./csim $s_size $b_size $byte_s $wp $wt $cache < traces/swim.trace > swim_output/$output_file"
                            ./csim $s_size $b_size $byte_s $wp $wt $cache < traces/swim.trace > swim_output/$output_file
                            echo "Done. Output saved to $output_file"
                    done
                done
            done
